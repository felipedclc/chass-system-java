...
public class ChessMatch { // CLASSE ONDE CONTEM AS REGRAS DO JOGO
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if(!type.equals("B") && !type.equals("N") && !type.equals("R") && !type.equals("Q")) { // EQUALS COMPARA SE UMA CLASSE(STRING) É IGUAL A OUTRA
			throw new InvalidParameterException("Invalid type for promotion");
		}
		
		Position pos = promoted.getChessPosition().toPosition(); // PEGANDO A POSIÇÃO DA PEÇA PROMOVIDA
		Piece p = board.removePiece(pos); // REMOVENDO A PEÇA POS E GUARDANDO NA VARIAVEL P
		piecesOnTheBoard.remove(p); // EXCLUINDO A PEÇA DA LISTA DE PEÇAS NO TABULEIRO
		
		ChessPiece newpPiece = newPiece(type, promoted.getColor());
		board.placePiece(newpPiece, pos); // COLOCANDO A PEÇA NO LUGAR DA REMOVIDA(POS)
		piecesOnTheBoard.add(newpPiece); // ADD NA LISTA
		
		return newpPiece;
	}
	
	private ChessPiece newPiece(String type, Color color) { // METODO AUXILIAR
